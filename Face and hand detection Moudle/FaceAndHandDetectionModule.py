        ...
        if results.multi_hand_landmarks: 
            for handLms in results.multi_hand_landmarks:
                if draw:
                    ...
                    self.mpDraw.DrawingSpec(color=(0,0,0), circle_radius=6, cv.FILLED),
                    self.mpDraw.DrawingSpec(color=(255,255,255), thickness=1 , circle_radius=1))
        return img
...
class faceDetector():
    ...
    def findFace(self,img,draw = True): 
        ...
        if results.multi_face_landmarks:
            for faceLms in results.multi_face_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img,faceLms,self.mpFace.FACE_CONNECTIONS,self.mpDraw.DrawingSpec(color=(255,255,255), thickness=1 , circle_radius=1))
        return img
